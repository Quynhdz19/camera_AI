...
def detect_persons_async(frame_small):
    ...
    try:
        # HOG detection với settings nhanh
...
def draw_detections(frame, scale_x, scale_y):
    """Draw detection boxes with large visible text"""
    for idx, ((x, y, w, h), weight) in enumerate(detection_boxes):
        # Scale to display size
        x, y, w, h = int(x * scale_x), int(y * scale_y), int(w * scale_x), int(h * scale_y)
        
        # Draw bounding box - MÀU VÀNG NEON rõ ràng
        ...
        # Draw corners (dễ nhìn hơn)
        corner_len = 30
        # Top-left
        cv2.line(frame, (x, y), (x + corner_len, y), (0, 255, 0), 4)
        cv2.line(frame, (x, y), (x, y + corner_len), (0, 255, 0), 4)
        # Top-right
        cv2.line(frame, (x + w, y), (x + w - corner_len, y), (0, 255, 0), 4)
        cv2.line(frame, (x + w, y), (x + w, y + corner_len), (0, 255, 0), 4)
        # Bottom-left
        cv2.line(frame, (x, y + h), (x + corner_len, y + h), (0, 255, 0), 4)
        cv2.line(frame, (x, y + h), (x, y + h - corner_len), (0, 255, 0), 4)
        # Bottom-right
        cv2.line(frame, (x + w, y + h), (x + w - corner_len, y + h), (0, 255, 0), 4)
        cv2.line(frame, (x + w, y + h), (x + w, y + h - corner_len), (0, 255, 0), 4)
        
        # Label với background để dễ đọc
